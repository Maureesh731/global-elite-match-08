...
serve(async (req) => {
  ...
  try {
    ...
    if (hasActiveSub) {
      ...
      isFirstMonth = subscriptionStart > oneMonthAgo;
      logStep("Active subscription found", { subscriptionId: subscription.id, endDate: subscriptionEnd, isFirstMonth });
    } else {
    ...
    const referralCredits = referralData?.length || 0;
